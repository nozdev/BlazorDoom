...
namespace ManagedDoom.Audio
{
  public sealed class SfmlMusic : IMusic, IDisposable
  {
    ...
    IJSRuntime JSRuntime { get; set; }
     public SfmlMusic(IJSRuntime _JSRuntime, Config config, Wad wad, string sfPath)
    {
      try
      {
        ...
        stream = new MusStream(JSRuntime, this, config, sfPath);
        current = Bgm.NONE;
    ...
    private class MusStream
    {
      ...
      public MusStream(IJSRuntime _JSRuntime, SfmlMusic parent, Config config, string sfPath)
      {
        JSRuntime = _JSRuntime;
        this.parent = parent;
        this.config = config;
      ...
      public bool OnGetData(out short[] samples)
      {
        ...
        int[] intSamples = Array.ConvertAll(batch, Convert.ToInt32);
                JSRuntime.InvokeAsync<string>("playMusic", intSamples, MusDecoder.SampleRate, 0);
